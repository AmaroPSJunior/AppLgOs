...
async function syncToGithub() {
  ...
  try {
    ...
    const { data: newCommitData } = await octokit.rest.git.createCommit({
      owner,
      repo,
      message: `Sync: webOS Vanilla App Final Fix - ${new Date().toLocaleString()}`,
      tree: treeData.sha,
    ...
    try {
      if (latestCommitSha) {
       ...
       console.log("ℹ️ Tentativa de criação de ref fallback...");
       await octokit.rest.git.createRef({
         owner,
         repo,
         ref: "refs/heads/main",
         sha: newCommitData.sha,
       }).catch(() => console.log("⚠️ Branch já existe ou erro ignorável."));
    }
